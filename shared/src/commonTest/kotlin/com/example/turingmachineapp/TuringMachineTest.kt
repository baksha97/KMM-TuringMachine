...
class TuringMachineTest {
    ...
    fun testMachine() {
        ...
        val program = factory.makeProgram(programInput)

        val input = 5
        val tape = Tape(600, listOf(input))
        ...
        while (machine.hasSubsequentState()){
            ...
            println(mutableListOf<Char>()
                .apply {
                    addAll(machine.tape.reel)
                    set(machine.tape.reelPosition, '^')
                }
            )
            println(machine.tape.calculateIntegersOnReel())
